...
router.get(`/`, (request, response) => {
  ...
  if (requestProperty == "limit") {
    ...
    //TEST URL: http://localhost:8000/api/v1/pokemons?limit=20&offset=0
    ...
    // GET all pokemons
    // TEST URL: http://localhost:8000/api/v1/pokemons/
...
router.get(`/search`, (request, response) => {
  ...
  if (requestProperty === "type") {
    // type query
    // TEST URL: http://localhost:8000/api/v1/pokemons/search?type=Fire
    ...
    // name query
    // TEST URL: http://localhost:8000/api/v1/pokemons/search?name=Ivysaur
    ...
    // without query
    // TEST URL: http://localhost:8000/api/v1/pokemons/search?
    Pokemon.find()
      .then((res) => {
        response.status(200).send({ pokemons: res });
      })
...
// TEST URL: http://localhost:8000/api/v1/pokemons/Ivysaur
